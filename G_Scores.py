...
for iTrial in range(0, nTrials):
    ...
    # get the index of one note - key mapping
    iMapping = random.randint(0, len(notes))
    ...
    notes[iTrial].stimuli.present(clear=False, update=True)


    # Wait for button press
    exp.keyboard.wait(eval("constants.K_" + notes[iTrial].keyboard))

    # Add feedback to the screen
    text_mapping = stimuli.TextBox(notes[iTrial].key, [80, 80], position=[0, (y_init + line_dist)], text_size=18)
